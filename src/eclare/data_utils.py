...
def fetch_data_from_loader_light(loader, subsample=2000, label_key='cell_type', batch_key='batch', shuffle=True):
    ...
    labels = loader.dataset.obs[label_key].values[cells_idx].tolist()
    batches = loader.dataset.obs[batch_key].values[cells_idx].tolist() if batch_key in loader.dataset.obs.columns else np.nan
