...
echo "Job ID: multiVI_$JOB_ID"

rm -rf /home/mcb/users/dmannk/scMultiCLIP/outputs/multiVI_${JOB_ID} && mkdir /home/mcb/users/dmannk/scMultiCLIP/outputs/multiVI_${JOB_ID}
TMPDIR=/home/mcb/users/dmannk/scMultiCLIP/outputs/multiVI_${JOB_ID}
...
cp ${PWD}/benchmark_vertical/multiVI/multiVI_run.py ${PWD}/MCB_bash_scripts/benchmark_vertical/multiVI_run.sh $TMPDIR
...
for target_dataset in "${datasets[@]}"; do
    ...
    for source_dataset in "${datasets[@]}"; do
        ...
        if [ "$source_dataset" != "$target_dataset" ]; then
            ...
            CUDA_VISIBLE_DEVICES=${idle_array[${i}]} python benchmark_vertical/multiVI/multiVI_run.py \
