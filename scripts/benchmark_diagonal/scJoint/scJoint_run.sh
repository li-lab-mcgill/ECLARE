...
cd /home/dmannk/projects/def-liyue/dmannk/CLARE/benchmark_diagonal/scJoint/
source ${PWD}/scjoint_env/bin/activate
 
TMPDIR=/home/dmannk/scratch/scjoint_${SLURM_JOB_ID}
mkdir $TMPDIR

mkdir -p /home/dmannk/scratch/scJoint_data_tmp
...
for target_dataset in "${datasets[@]}"; do
    ...
    for source_dataset in "${datasets[@]}"; do
        ...
        if [ "$source_dataset" != "$target_dataset" ]; then
            ...
            srun python ${PWD}/scJoint_run.py --outdir $TMPDIR/$target_dataset/$source_dataset/ \
