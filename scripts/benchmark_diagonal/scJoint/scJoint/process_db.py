...
def h5_reader(file_name):
	...
	with h5py.File(file_name, 'r') as f:
		h5 = f['matrix']
		h5_data = h5['data'][:]
		h5_indices = h5['indices'][:]
		h5_indptr = h5['indptr'][:]
		h5_shape = h5['shape'][:]

	h5_data_csr = scipy.sparse.csr_matrix((h5_data, h5_indices, h5_indptr), shape=h5_shape)
	h5_data_csr = h5_data_csr.T

	print('H5 dataset shape:', h5_data_csr.shape)
	
	return h5_data_csr
	
	
def to_sparse_mat(file_name):
	sparse_data = h5_reader(file_name)
