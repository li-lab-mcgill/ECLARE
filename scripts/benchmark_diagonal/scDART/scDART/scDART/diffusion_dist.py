...
def phate_similarity_2(T, t):

    from sklearn.metrics import pairwise_distances

    T_t = torch.linalg.matrix_power(T, t)
    U_t = - torch.log(T_t + 1e-7)

    # calculate pairwise feature vector distance
    dist = torch.cdist(U_t, U_t)
    return dist
...
def diffu_distance(data, n_neigh = 5, ts = [30,40,50,60], use_potential = False, dr = "lsi", n_components = 100, method = "exact", n_anchor = None, **kwargs):
    ...
    for t in ts:
        phate_operator = phate.PHATE(knn=5, decay=20, t=t, gamma=1, n_pca=30, n_landmark=None)
        T = phate_operator.fit(data).diff_op
        T = torch.tensor(T, dtype=torch.float32, device=os.environ['device'])

        diffu_t = phate_similarity_2(T, t)
        diffu_t = diffu_t/torch.linalg.norm(diffu_t)
        diffu_t = diffu_t.cpu().numpy()
        diffu += diffu_t
    # average
    diffu = diffu/len(ts)
    return diffu
