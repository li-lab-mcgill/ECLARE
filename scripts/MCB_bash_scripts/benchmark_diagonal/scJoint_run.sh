...
echo "Job ID: scJoint_$JOB_ID"

rm -rf /home/mcb/users/dmannk/scMultiCLIP/outputs/scJoint_${JOB_ID} && mkdir /home/mcb/users/dmannk/scMultiCLIP/outputs/scJoint_${JOB_ID}
TMPDIR=/home/mcb/users/dmannk/scMultiCLIP/outputs/scJoint_${JOB_ID}

exec > >(tee -a ${TMPDIR}/log.txt) 2>&1

conda init
conda activate scjoint_env
cd /home/mcb/users/dmannk/scMultiCLIP/CLARE

## Copy scripts to sub-directory for reproducibility
cp ${PWD}/benchmark_diagonal/scJoint/scJoint_run.py ${PWD}/MCB_bash_scripts/benchmark_diagonal/scJoint_run.sh $TMPDIR
...
mkdir -p /home/mcb/users/dmannk/scMultiCLIP/outputs/scJoint_data_tmp
    ...
    for source_dataset in "${datasets[@]}"; do
        ...
        if [ "$source_dataset" != "$target_dataset" ]; then
            ...
            seq 0 $((N - 1)) | xargs -n1 -P$N -I{} bash -c '
                ...
                ## Run python script
                CUDA_VISIBLE_DEVICES=${idle_array[${i}]} python ${PWD}/benchmark_diagonal/scJoint/scJoint_run.py \
