...
## subset RNA data for training UMAP
n_cells = student_rna.n_obs
n_splits = np.ceil(n_cells / subsample).astype(int)

skf = StratifiedKFold(n_splits=n_splits, shuffle=False)
combinations = student_rna.obs[[cell_group, dev_group_key]].apply(lambda x: ' - '.join(x), axis=1).values
_, cells_idx = next(skf.split(np.zeros_like(combinations), combinations))

student_rna_sub = student_rna[cells_idx].copy()

## subset ATAC data for training UMAP
n_cells = student_atac.n_obs
n_splits = np.ceil(n_cells / subsample).astype(int)

skf = StratifiedKFold(n_splits=n_splits, shuffle=False)
combinations = student_atac.obs[[cell_group, dev_group_key]].apply(lambda x: ' - '.join(x), axis=1).values
_, cells_idx = next(skf.split(np.zeros_like(combinations), combinations))

student_atac_sub = student_atac[cells_idx].copy()
