...
class TrainingProcessStage3():
    def __init__(self, config, idx_dict):
        self.config = config
        # load data
        self.train_rna_loaders, self.test_rna_loaders, self.train_atac_loaders, self.test_atac_loaders, self.training_iters = PrepareDataloader(config, idx_dict).getloader()
    ...
    def write_embeddings(self):
        ...
        for i, rna_loader in enumerate(self.test_rna_loaders):
            ...
            fp_em = open(os.environ['outdir'] + db_name + '_embeddings.txt', 'w')
            for batch_idx, rna_data in enumerate(rna_loader):    
                # prepare data
                rna_label = torch.empty(0)
                rna_data, rna_label = prepare_input([rna_data, rna_label], self.config)
        ...
        for i, atac_loader in enumerate(self.test_atac_loaders):
            ...
            fp_em = open(os.environ['outdir'] + db_name + '_embeddings.txt', 'w')
            fp_pre = open(os.environ['outdir'] + db_name + '_predictions.txt', 'w')
            for batch_idx, atac_data in enumerate(atac_loader):    
                # prepare data
                atac_label = torch.empty(0)
                atac_data, atac_label = prepare_input([atac_data, atac_label], self.config)
