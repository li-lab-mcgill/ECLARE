...
cd /home/dmannk/projects/def-liyue/dmannk/CLARE/benchmark_vertical/multiVI/
source ${PWD}/scvi_env/bin/activate
 
TMPDIR=/home/dmannk/scratch/multiVI_${SLURM_JOB_ID}
mkdir $TMPDIR
...
for target_dataset in "${datasets[@]}"; do
    ...
    for source_dataset in "${datasets[@]}"; do
        ...
        if [ "$source_dataset" != "$target_dataset" ]; then
            ...
            srun python ${PWD}/multiVI_run.py --outdir $TMPDIR/$target_dataset/$source_dataset \
